...
import { Form, Input, InputNumber, Button, Select, Switch, message, Space, Tooltip } from 'antd';
...
const ProductEdit: React.FC = () => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        const additionalImages = productData.images
          ? productData.images.filter((img: string) => img !== mainImage)
          : [];
  ...
  const onFinish = async (values: Product & { additionalImages?: string[] }) => {
    try {
      ...
      await axios.put(`http://localhost:4000/products/${id}`, productData);
  ...
  return (
    <div className="p-6 bg-white h-screen overflow-auto">
      ...
      <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ additionalImages: [] }}>
        ...
        >
          <Select placeholder="Chọn màu sắc" optionLabelProp="label">
      ...
      {previewVisible && imagePreview && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50" onClick={handleClosePreview}>
          <div className="max-w-3xl max-h-[80vh] overflow-auto bg-white p-2" onClick={(e) => e.stopPropagation()}>
            <img src={imagePreview || "/placeholder.svg"} alt="Preview" className="max-w-full max-h-[70vh] object-contain" />
